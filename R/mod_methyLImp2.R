...
#' from one chromosome.
mod_split_by_chromosomes <- function(
    data,
  ...
  type <- match.arg(type)
  if (type == "450K") {
    utils::data("hm450.manifest.hg19",
      package = "ChAMPdata",
      envir = environment()
    )
    anno <- data.frame(
      cpg = row.names(hm450.manifest.hg19),
      chr = hm450.manifest.hg19$CpG_chrm
    )
    remove(hm450.manifest.hg19)
  } else if (type == "EPIC") {
    utils::data("EPIC.manifest.hg19",
      package = "ChAMPdata",
      envir = environment()
    )
    anno <- data.frame(
      cpg = row.names(EPIC.manifest.hg19),
  ...
  stopifnot("in annotation, the cpg name has to be unique" = length(anno$cpg) == length(unique(anno$cpg)))
  ...
  # (if such exist)
  data_chr <- data_chr[names(data_chr)[vapply(
    data_chr,
    function(x) {
      dim(x)[2]
    }, numeric(1)
  ...
  chr_order <- names(sort(
    vapply(
      data_chr, function(x) dim(x)[2],
      numeric(1)
    ),
    decreasing = TRUE
  ))
...
#' Impute Missing Values in DNA Methylation Data
#'
#' @description
...
#' are considered.
#' @param parallel a boolean specifying to run in parallel or not.
#' @param progress a boolean specifying to show progress bar or not.
...
mod_methyLImp2 <- function(
  ...
  # check `skip_imputation_ids`
  if (!is.null(skip_imputation_ids)) {
    if (!is.character(skip_imputation_ids)) {
      stop("`skip_imputation_ids` should be a vector of unique characters without NA.")
    }
    if (anyNA(skip_imputation_ids) || length(skip_imputation_ids) != length(unique(skip_imputation_ids))) {
      stop("`skip_imputation_ids` should not contain NA and should have unique values.")
    }
    if (!all(skip_imputation_ids %in% colnames(data_full))) {
      stop("All `skip_imputation_ids` should be in the column names of the input matrix.")
    }
  }
  ...
  for (i in seq_len(ngroups)) {
    ...
    if (sum(vapply(res, function(x) is.character(x), logical(1))) > 0) {
      problem_chr <- names(data_chr)[vapply(
        res, function(x) is.character(x),
        logical(1)
      )]
      for (i in 1:length(problem_chr)) {
        message(
          "Sample group ", curr_group, ", ",
    ...
    for (c in seq_len(nchr)) {
      ...
      imputed <- res[[c]]
      rows_id <- row.names(data_group) %in% row.names(imputed)
  ...
  if (flag == "SE") {
    if (overwrite_res) {
      ...
      assay(input, paste(which_assay, "_imputed", sep = "_")) <- t(out_full)
      return(input)
    }
  } else {
    return(out_full)
...
#' Process NA
#'
#' Generate is.na(data) matrix and remove CpGs that are skipped or have no/all NA or only 1 non-NA
#'
#' @inheritParams mod_methyLImp2_internal
#'
#' @noRd
#' @keywords internal
...
#'
#' This function processes a logical matrix representing missing data to identify
#' unique patterns of missingness. It groups together columns (variables) that
#' share the same missing data pattern and identifies the rows (samples) where
#' data is missing for each pattern.
#'
#' @param dat_na A logical matrix where \code{TRUE} indicates missing data (NA)
#' and \code{FALSE} indicates present data. Rows correspond to samples, and columns
